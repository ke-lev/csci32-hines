...
describe('getClosestTimelinePost', () => {
  ...
  it('picks the latest post already present when the date falls between entries', () => {
    expect(getClosestTimelinePost(posts, new Date('2026-08-20T00:00:00Z'))?.date).toBe('2026-08-17')
    expect(getClosestTimelinePost(posts, new Date('2026-08-29T00:00:00Z'))?.date).toBe('2026-08-17')
  })
  ...
  it('falls back to the first post before the semester begins', () => {
    expect(getClosestTimelinePost(posts, new Date('2026-08-01T00:00:00Z'))?.date).toBe('2026-08-17')
  })
