...
export function PageIntro({ body, children, subhead, title, titleId }: PageIntroProps) {
  return (
    <div className="self-center">
      ...
      <p className="mt-11 max-w-[530px] text-[clamp(1rem,1.3vw,1.2rem)] leading-[1.55] text-subhead text-balance max-[900px]:mt-[34px]">
        {subhead}
      </p>
      {body && <p className="mt-5 max-w-[52ch] text-sm leading-[1.6] text-muted">{body}</p>}
      {children}
    </div>
  )
}
